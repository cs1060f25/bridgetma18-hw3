...
/**
 * Unified AI Workspace — single-file prototype
 * -------------------------------------------------
 ...
 * 3) Professor-created AI templates usable by all enrolled students (with guardrails + usage logs)
 * 4) Smart scheduling that intersects student availability with office hours
 *
 * Notes
 * - Tailwind classes are used for styling (no external deps required).
 * - All data is local mock data. Swap in your backend later.
 * - Templates use {{variable}} placeholders (simple replacement engine below).
 */
...
// Example student weekly availability (local, editable)
...
// Naive extractive syllabus summarizer: pulls key lines + bullets
function summarizeSyllabus(text) {
  const lines = text.split(/\n|\r/).map((s) => s.trim()).filter(Boolean);
  const sections = {
    Description: [], Goals: [], Grading: [], Attendance: [], OfficeHours: [], LatePolicy: [], AIPolicy: [], Accessibility: [], Misc: [],
  };
...
// Compute next N upcoming course items (assignments + announcements)
function useUpcoming(courses, count = 8) {
  return useMemo(() => {
    const items = [];
    for (const c of courses) {
      for (const a of c.assignments) items.push({
        type: "Assignment",
        course: c,
        title: a.title,
        date: a.due,
      ...
      for (const an of c.announcements) items.push({
        type: "Announcement",
        course: c,
        title: an.title,
        date: an.date,
...
// Intersect availability with OH (and avoid busy events)
function findOHMatches(availability, officeHours, busy = []) {
  ...
  for (const oh of officeHours) {
    ...
    let ok = false;
    // Check student availability windows
    for (const r of dayAvail.ranges) {
      if (overlaps(r.start, r.end, oh.start, oh.end)) {
        ok = true;
        break;
      }
    }
...
// ------------------------------ UI Bits ------------------------------------
// Week calendar constants and helpers
  ...
  return (
    <div className="absolute inset-0">
      {hours.map(function(h, i){
        return (
          <div key={i} className="absolute left-0 right-0 border-t border-slate-100 text-[10px] text-slate-400" style={{ top: (h - START_HOUR) * 60 * PX_PER_MIN }}>
...
function Block(props) {
  return (
    <div className={"absolute left-1 right-1 rounded-lg border border-indigo-600/30 bg-white/70 backdrop-blur-sm shadow-sm overflow-hidden"}
         style={{ top: props.top, height: props.height }}>
      <div className={"h-full w-full p-2 text-xs leading-4 " + (props.colorClass || "bg-indigo-500/20")}>{props.children}</div>
    </div>
  );
}

function WeekCalendar(props) {
  var officeHours = props.officeHours || [];
  var events = props.events || [];
  var showOnlyFree = !!props.showOnlyFree;

  var annotatedOH = officeHours.map(function(oh){
    var conflicts = events.filter(function(e){ return e.day === oh.day && overlaps(e.start, e.end, oh.start, oh.end); });
    return Object.assign({}, oh, { conflict: conflicts.length > 0 });
  });

  var columns = days.map(function(d){
    return { day: d, oh: annotatedOH.filter(function(o){ return o.day === d; }), ev: events.filter(function(e){ return e.day === d; }) };
  });
  ...
  return (
    <div className="w-full overflow-x-auto">
      <div className="min-w-[900px] grid grid-cols-8 gap-2">
        <div />
        {days.map(function(d){ return <div key={d} className="text-sm font-medium text-slate-700 text-center">{d}</div>; })}
          ...
          {columns.map(function(col){
            return (
              <div key={col.day} className="relative bg-white rounded-xl border border-slate-200" style={{ height: (END_HOUR - START_HOUR) * 60 * PX_PER_MIN }}>
                <WeekGridLines />
                {col.ev.map(function(e){
                  var pos = positionFor(e.start, e.end);
                  return (
                    <Block key={e.id} top={pos.top} height={pos.height} colorClass="bg-slate-500/20" >
                      <div className="font-medium text-slate-700">{e.title}</div>
                      <div className="text-slate-500">{formatTime(e.start)}–{formatTime(e.end)}</div>
                    </Block>
                  );
                })}
                {col.oh.filter(function(o){ return showOnlyFree ? !o.conflict : true; }).map(function(o){
                  var pos2 = positionFor(o.start, o.end);
                  var color = o.conflict ? "bg-red-500/20" : "bg-indigo-500/20";
                  return (
                    <Block key={o.courseId + "-" + o.id} top={pos2.top} height={pos2.height} colorClass={color}>
                      <div className="flex items-center justify-between">
                        ...
                        {!o.conflict && (
                          <button onClick={function(){ props.onBook && props.onBook(o); }} className="ml-2 shrink-0 px-2 py-1 rounded-md bg-indigo-600 text-white text-[11px]">Book</button>
                        )}
                      </div>
                    </Block>
                  );
...
function Tag({ children }) {
  return <span className="inline-block rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-700 border border-slate-200">{children}</span>;
}
...
function PillButton({ children, onClick, variant = "primary" }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${styles}`}
    >
      {children}
    </button>
  );
...
function TextArea({ value, onChange, rows = 6, placeholder }) {
  return (
    <textarea
      className="w-full rounded-xl border border-slate-200 p-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500"
      value={value}
...
export default function UnifiedAIWorkspace() {
    ...
    setUsageLog((log) => [
      {
        ...
        settings: { citeAI: guardCiteAI, highVarianceOff: guardHighVariance },
      },
      ...log,
    ]);

    // Decrement usage budget in template guardrails
    setCourses((prev) =>
      prev.map((c) =>
        c.id !== course.id
          ? c
              ...
              templates: c.templates.map((t) =>
                t.id !== tpl.id
                  ...
                  : {
                      ...
                      guardrails: {
                        ...t.guardrails,
                        usageBudget: Math.max(0, (t.guardrails?.usageBudget ?? 0) - 1),
                      },
                    }
              ),
            }
  ...
  function addRange(day) {
    setAvailability((prev) =>
      prev.map((d) => (d.day === day ? { ...d, ranges: [...d.ranges, { start: "13:00", end: "15:00" }] } : d))
    );
  }
  function updateRange(day, idx, key, value) {
    setAvailability((prev) =>
      prev.map((d) =>
        d.day !== day
          ? d
  ...
  function removeRange(day, idx) {
    setAvailability((prev) =>
      prev.map((d) => (d.day !== day ? d : { ...d, ranges: d.ranges.filter((_, i) => i !== idx) }))
    );
  }
    ...
    return (
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 space-y-4">
          <Card
            title="Unified Upcoming View"
            right={<Tag>{new Date().toLocaleDateString()}</Tag>}
          >
        ...
        <div className="space-y-4">
          <Card title="Office Hours (All Courses)">
            <div className="space-y-2">
              {allOH.map((oh) => (
                <Row
                  key={`${oh.courseId}-${oh.id}`}
                  left={<span>{oh.day} {formatTime(oh.start)}–{formatTime(oh.end)} <span className="text-slate-500">· {oh.courseName}</span></span>}
                  right={<span className="flex items-center gap-2"><Tag>{oh.staff}</Tag><Tag>{oh.channel}</Tag><span className="text-slate-400">{oh.location}</span></span>}
                />
              ))}
            </div>
          </Card>
          ...
          <Card title="AI Usage Log (Accountability)">
            <div className="space-y-2 max-h-72 overflow-auto">
              ...
              {usageLog.map((u) => (
                <Row
                  key={u.id}
                  left={<span className="text-slate-700">{u.template} <span className="text-slate-400">· {u.courseName}</span></span>}
                  right={<span className="text-slate-500">{formatDateTimeLocal(u.timestamp)}</span>}
                  sub={`Settings: citeAI=${u.settings.citeAI ? "on" : "off"}, highVarianceOff=${u.settings.highVarianceOff ? "on" : "off"}`}
                />
              ))}
            </div>
          </Card>
  ...
  function SyllabusSummarizer() {
    ...
    function run() {
      const text = selectedCourse?.syllabus ?? "";
      const out = summarizeSyllabus(text);
      setSummary(out);
    }
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card
          title={`Syllabus · ${selectedCourse.code}`}
          right={<Tag>{selectedCourse.instructor}</Tag>}
        >
          <pre className="whitespace-pre-wrap text-sm text-slate-800 leading-relaxed">{selectedCourse.syllabus}</pre>
        </Card>
        <Card
          title="AI-Generated Summary (extractive)"
          right={<PillButton onClick={run}>Summarize</PillButton>}
        >
          {summary ? (
        ...
        <Card title={`Templates · ${selectedCourse.code}`}>
          <div className="space-y-2">
            {selectedCourse.templates.map((t) => (
              <button
                key={t.id}
                onClick={() => { setActiveTemplateId(t.id); setVariables({}); setOutput(""); }}
                className={`w-full text-left px-3 py-2 rounded-xl border transition ${
                  t.id === activeTemplateId ? "bg-indigo-50 border-indigo-200" : "hover:bg-slate-50"
                }`}
              >
    ...
    return (
      <div className="space-y-4">
        <Card title="My Calendar · Add Events">
          <EventEditor />
          <div className="mt-3 text-xs text-slate-500">This simulates Google Calendar data. Later, connect Calendar APIs and auto-sync.</div>
        </Card>
  ...
  function EventEditor() {
    ...
    function submit() {
      ...
      if (toMinutes(form.end) <= toMinutes(form.start)) { setError('End must be after start'); return; }
      setUserEvents(function(prev){ return [{ id: 'evt_' + Date.now(), title: form.title, day: form.day, start: form.start, end: form.end },].concat(prev); });
      setForm({ ...form, title: '' });
    }
    return (
      <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
        <input className="rounded-xl border p-2 text-sm md:col-span-2" placeholder="Event title (e.g., Math Lecture)" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
        <select className="rounded-xl border p-2 text-sm" value={form.day} onChange={(e) => setForm({ ...form, day: e.target.value })}>{days.map((d) => <option key={d}>{d}</option>)}</select>
        <input type="time" className="rounded-xl border p-2 text-sm" value={form.start} onChange={(e) => setForm({ ...form, start: e.target.value })} />
        <input type="time" className="rounded-xl border p-2 text-sm" value={form.end} onChange={(e) => setForm({ ...form, end: e.target.value })} />
  ...
  function Scheduler() {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  ...
                  {d.ranges.map((r, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <input
                        type="time"
                        value={r.start}
                        onChange={(e) => updateRange(d.day, idx, "start", e.target.value)}
                      ...
                      <input
                        type="time"
                        value={r.end}
                        onChange={(e) => updateRange(d.day, idx, "end", e.target.value)}
                        className="rounded-lg border p-1 text-sm"
                      />
        ...
        <Card title="Matching Office Hours">
          <div className="space-y-2 max-h-[460px] overflow-auto">
            {matches.length === 0 && <div className="text-sm text-slate-500">No matches yet. Adjust your availability.</div>}
            {matches.map((oh, i) => (
        ...
        <Card title="My Events (Conflicts)">
          <div className="space-y-2">
            {userEvents.map((b) => (
              <Row key={b.id} left={`${b.title}`} right={`${b.day} ${formatTime(b.start)}–${formatTime(b.end)}`} />
            ))}
  ...
  function FacultyPanel() {
    ...
    function addTemplate() {
      const vars = newTpl.variables
        .split(/,|\s+/)
        .map((v) => v.trim())
        .filter(Boolean);
      const tpl = {
        id: `tpl_${Date.now()}`,
        name: newTpl.name || "Untitled Template",
        content: newTpl.content || "",
        variables: vars,
        guardrails: { citeAI: true, usageBudget: Number(newTpl.budget) || 8 },
      };
      setCourses((prev) =>
        prev.map((c) =>
          c.id === selectedCourseId ? { ...c, templates: [tpl, ...c.templates] } : c
        )
      );
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                placeholder="e.g., Lab Writeup Assistant"
              />
            </div>
            <div>
              ...
              <TextArea
                ...
                onChange={(v) => setNewTpl((s) => ({ ...s, content: v }))}
                rows={8}
                placeholder="Summarize {{paperTopic}} in 5 bullets..."
              />
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2">
                ...
                <input
                  ...
                  placeholder="paperTopic, dueDate, pages"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 p-2 text-sm"
                  value={newTpl.budget}
                  onChange={(e) => setNewTpl((s) => ({ ...s, budget: e.target.value }))}
                />
              </div>
            </div>
          ...
          .map((tab) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-sm font-medium border transition ${
                activeTab === tab ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50"
              }`}
            >
              {tab}
            </button>
